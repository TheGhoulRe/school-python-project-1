from tkinter import Label, Text, Button, Listbox
...
def setup_ui(root):
    ...
    student_form          = StudentForm("students.csv", name, mat_no, score)
    eda_engine            = EDAEngine(exploratory_analysis, s_above_70_widget, student_form)

    setup_save_student_button(root, student_form)
...
def setup_save_student_button(root, student_form):
    Button(
        root,
        text="save student score",
        font=("regular", 13),
        command=student_form.add_student,
...
def setup_perform_eda_button(root, eda_engine):
    Button(
        ...
        font=("regular", 10),
        command=eda_engine.perform_eda
